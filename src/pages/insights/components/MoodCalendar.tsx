...
export function MoodCalendar({
  ...
  const prevMonthDays = firstDayOfMonth > 0 ? eachDayOfInterval({
  ...
  return <Card className="w-full h-full">
      <CardHeader className="pb-2">
        <div className="flex flex-col items-start gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2 mx-[30px]">
      ...
      <CardContent>
        ...
        <div className="grid grid-cols-7 gap-1 mb-1">
          {weekDays.map(day => <div key={day} className="text-center text-xs font-medium text-muted-foreground">
              {day}
            </div>)}
        </div>
        ...
        <div className="grid grid-cols-7 gap-1">
          ...
          return <div key={`prev-${index}`} className={`aspect-square rounded-full flex items-center justify-center text-xs font-medium
                  ${colorClass} opacity-40`} title={mood ? `${format(day, 'PP')}: ${mood.value}` : format(day, 'PP')}>
                {day.getDate()}
              </div>;
        })}
          ...
          return <div key={index} className={`aspect-square rounded-full flex items-center justify-center text-xs font-medium
                  ${colorClass} ${isToday ? 'ring-2 ring-mindscape-primary' : ''}`} title={mood ? `${format(day, 'PP')}: ${mood.value}` : format(day, 'PP')}>
                {day.getDate()}
              </div>;
        })}
          ...
          {Array.from({
          length: (7 - (prevMonthDays.length + days.length) % 7) % 7
        }).map((_, index) => {
          ...
          const colorClass = getMoodColorClass(mood);
          return <div key={`next-${index}`} className={`aspect-square rounded-full flex items-center justify-center text-xs font-medium
                  ${colorClass} opacity-40`} title={mood ? `${format(nextMonthDay, 'PP')}: ${mood.value}` : format(nextMonthDay, 'PP')}>
